...
unsigned long send_period_dist = 3000;
int packets_per_period_dist = 10;
unsigned long tx_wait_time_dist = 10000;
...
void loop()
{
  ...
  if ( (packet_number - 1) % packets_per_period_dist == 0) {
    ...
    while(millis() < wait_tracker + tx_wait_time_dist) 1;
  }
